...
func _init() -> void:
	...
	PopochiuUtils.e.register_command(Commands.WALK_TO, "Ir a ", walk_to)
	PopochiuUtils.e.register_command(Commands.OPEN, "Abrir ", open)
	PopochiuUtils.e.register_command(Commands.PICK_UP, "Coger ", pick_up)
	PopochiuUtils.e.register_command(Commands.PUSH, "Empujar ", push)
	PopochiuUtils.e.register_command(Commands.CLOSE, "Cerrar ", close)
	PopochiuUtils.e.register_command(Commands.LOOK_AT, "Mirar ", look_at)
	PopochiuUtils.e.register_command(Commands.PULL, "Tirar ", pull)
	PopochiuUtils.e.register_command(Commands.GIVE, "Dar ", give)
	PopochiuUtils.e.register_command(Commands.TALK_TO, "Hablar a ", talk_to)
	PopochiuUtils.e.register_command(Commands.USE, "Usar ", use)
...
func look_at() -> void:
	...
	await PopochiuUtils.c.player.say("No veo nada especial.")


## Called when [code]E.current_command == Commands.PULL[/code] and [code]E.command_fallback()[/code]
## is triggered.
func pull() -> void:
	await PopochiuUtils.c.player.say("No se mueve.")
...
func _give_or_use(callback: Callable) -> void:
	if PopochiuUtils.i.active and PopochiuUtils.e.clicked:
		...
		await PopochiuUtils.c.player.say("¿Qué?")
...
func use_item_on(_item: PopochiuInventoryItem, _target: Node) -> void:
	PopochiuUtils.i.active = null
	await PopochiuUtils.c.player.say("No quiero hacer eso")
