...
const Data := preload('character_mateo_state.gd')

var state: Data = load("res://game/characters/mateo/character_mateo.tres")
@onready var mi_sprite: Sprite2D = $Sprite2D
@onready var mi_animador: AnimationPlayer = $AnimationPlayer
var ultima_direccion = "s"
var contador=0
func _ready() -> void:
	super()
	# Conectamos "manualmente" el motor de Popochiu con tu Sprite
	if "visual" in self:
