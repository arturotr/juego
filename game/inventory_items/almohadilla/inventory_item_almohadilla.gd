...
const Data := preload('inventory_item_almohadilla_state.gd')

var state: Data = load("res://game/inventory_items/almohadilla/inventory_item_almohadilla.tres")
...
func _on_click() -> void:
	if E.current_command == NineVerbCommands.Commands.LOOK_AT:
		await C.player.say("Es mi caja de tinta para el sello.")
		await C.player.say("Está abierta y el fieltro aún conserva un azul oscuro muy intenso.")
	else:
		E.command_fallback()
