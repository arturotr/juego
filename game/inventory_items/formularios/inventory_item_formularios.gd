...
const Data := preload('inventory_item_formularios_state.gd')

var state: Data = load("res://game/inventory_items/formularios/inventory_item_formularios.tres")
...
func _on_click() -> void:
	if E.current_command == NineVerbCommands.Commands.LOOK_AT:
		await C.player.say("Son las solicitudes de obra del obispado y los planos originales del templo.")
		await C.player.say("Sin estos permisos firmados, el consejo de la isla no me dejaría mover ni una piedra.")
	else:
		E.command_fallback()
