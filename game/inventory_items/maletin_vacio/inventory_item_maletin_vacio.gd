...
const Data := preload('inventory_item_maletin_vacio_state.gd')

var state: Data = load("res://game/inventory_items/maletin_vacio/inventory_item_maletin_vacio.tres")
...
func _on_click() -> void:
	if E.current_command == NineVerbCommands.Commands.LOOK_AT:
		await C.player.say("Ahora está vacío y pesa mucho menos. Ya he sacado todo mi equipo de trabajo.")
	else:
		E.command_fallback()
