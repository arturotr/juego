...
const Data := preload('inventory_item_regla_state.gd')

var state: Data = load("res://game/inventory_items/regla/inventory_item_regla.tres")
...
func _on_click() -> void:
	if E.current_command == NineVerbCommands.Commands.LOOK_AT:
		await C.player.say("Mi vieja regla de madera de haya. Tiene las marcas de los milímetros un poco desgastadas, pero sigue siendo precisa.")
	else:
		E.command_fallback()
