...
const Data := preload('inventory_item_lapiz_state.gd')

var state: Data = load("res://game/inventory_items/lapiz/inventory_item_lapiz.tres")
...
func _on_click() -> void:
	if E.current_command == NineVerbCommands.Commands.LOOK_AT:
		await C.player.say("Un lápiz de dibujo clásico, sin lujos. Está bien afilado.")
	else:
		E.command_fallback()
