...
const Data := preload('inventory_item_sello_state.gd')

var state: Data = load("res://game/inventory_items/sello/inventory_item_sello.tres")
...
func _on_click() -> void:
	if E.current_command == NineVerbCommands.Commands.LOOK_AT:
		await C.player.say("Es mi sello de arquitecto colegiado.")
		await C.player.say("Tiene un buen mango de madera, robusto y cómodo para estampar informes.")
	else:
		E.command_fallback()
